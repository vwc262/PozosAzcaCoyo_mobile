	...
	for child in meshes:
		if child is MeshInstance3D:
			if child.name.contains('_'):
				...
				base_transform_labels.origin = child.position + Vector3(0,0.7,0);
				...
				base_transform_poi.origin = child.position + Vector3(0, 1.948, 4.4)
				base_transform_poi.basis = Basis.from_euler(Vector3(deg_to_rad(-35.2),0,0))
